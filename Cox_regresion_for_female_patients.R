...
df1 <- readRDS(tcga_gbm_for_female.rds)

## remove the row which is "0"
tmp <- df1
tmp <- tmp[-290,] 
## survival time transformation
tmp[2:21] <- log2(tmp[2:21])
## cox regression for the candidate genes from PPI
formula_for_female <- as.formula(paste0('Surv(times, patient.vital_status)~', paste(colnames(tmp)[2:21], sep = '', collapse = '+')))
res.cox <- coxph( formula_for_female , data = tmp)
summary(res.cox)
## visualization of HR
ggforest(res.cox, data = tmp, main = 'Hazard ratios of candidate genes', fontsize = 1)

## c("ECT2","NMB","GPR65"),p < 0.05,this three genes can be selected for following analysis.
...
res.cox_female <- as.formula(paste0('Surv(times, patient.vital_status)~',paste0(c("ECT2","NMB","GPR65") , sep = ' ', collapse = '+')))
res.cox_female <- coxph(res.cox_female, data = tmp)
summary(res.cox_female)

## check the PH hypothesis.
ph_hypo_multi <- cox.zph(res.cox_female)
ph_hypo_multi 
ph_hypo_table <- ph_hypo_multi$table[-nrow(ph_hypo_multi$table),]

## examinate the co-linearity of the three genes
library('rms')
vif <- rms::vif(res.cox_female)
...
## remove the gene whose sqrt(vif) > 2 , so the two genes c("ECT2 ","GPR65") left!
res.cox_female <- as.formula(paste0('Surv(times, patient.vital_status)~', paste(c("ECT2","GPR65"), sep = '', collapse = '+')))
res.cox_female <- coxph(res.cox_female, data = tmp)
summary(res.cox_female)

## viusalization from the HR 
ggforest(res.cox_female, data = tmp, main = 'Hazard ratios of candidate genes', fontsize = 1)
vif <- rms::vif(res.cox_female)
...
risk_score_table_multi_cox2 <- riskscore(tmp, candidate_genes_for_cox2, res.cox_female)
